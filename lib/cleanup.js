...
import { TRASH_DIR, TRASH_MANIFEST, PROJECTS_DIR, migrateFromOldDir } from './utils.js';
...
async function loadManifest() {
  await migrateFromOldDir();
  try {
    const data = await readFile(TRASH_MANIFEST, 'utf-8');
    const parsed = JSON.parse(data);
    // Normalize: older versions used "sessions" key instead of "items"
    if (!Array.isArray(parsed.items)) {
      parsed.items = Array.isArray(parsed.sessions) ? parsed.sessions : [];
