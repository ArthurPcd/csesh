...
export function computeStats(sessions, projectFilter = null) {
  ...
  for (const s of filtered) {
    ...
    const day = s.firstTimestamp.slice(0, 10);
    const model = s.models[0] || 'default';
    const cost = estimateCost(s.tokenUsage, model);
  ...
  const costByProject = {};
  for (const s of filtered) {
    const p = s.shortProject || 'unknown';
    const model = s.models[0] || 'default';
    costByProject[p] = (costByProject[p] || 0) + estimateCost(s.tokenUsage, model);
  }

  // Top files (from filesMentioned / filesTouched)
  const fileCounts = {};
  for (const s of filtered) {
    const files = s.filesMentioned || s.filesTouched || [];
  ...
  // Thinking ratio (thinking blocks / total assistant messages)
  const totalThinkingBlocks = filtered.reduce((sum, s) => sum + (s.thinkingBlocks || 0), 0);
  const thinkingRatio = totalAssistantMessages > 0
    ? Math.round((totalThinkingBlocks / totalAssistantMessages) * 1000) / 1000
    : 0;
